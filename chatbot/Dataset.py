...
class ProcessChat():
	...
	def process(self,df):
		...
		def banned(x):
			names= ['Gruzzly-MoonGuard','Tshirtzbot-MoonGuard']
			for n in names:
				if x.lower()==n.lower():
					return 1
			return 0
...
class CollectData(ProcessChat):
	...
	def __init__(self):
		...
		self.chat_path = "../../../Applications/World of Warcraft/_retail_/WTF/Account/391294447#1/Moon Guard/Tshirtzbot/SavedVariables/Elephant.lua"
	...
	def collectchat(self):
		...
		elp_log = eleph['ElephantDBPerChar']['char']['Tshirtzbot - Moon Guard']['logs']['trade']['logs']
		for x in elp_log:
